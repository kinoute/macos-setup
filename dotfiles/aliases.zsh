...
# Check if we have exa, if not use std ls
if [[ ! -x "$(command -v exa)" ]]; then
  alias l="ls -lF ${colorflag}"
else
  alias l="exa -l -h --icons --classify -a"
fi
...
alias cleanup="find -E . -regex '.*(\.DS_Store|__pycache__).*' -ls -delete"
...
# URL-encode strings
alias urlencode='python -c "import sys, urllib.parse as ul; print(ul.quote_p    lus(sys.argv[1]));"'
